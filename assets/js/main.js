(function ($) {
  ...
  */
  ...
  $(window).on("scroll resize", function () {
    if ($(window).width() <= 768) { 
      stickyMenuMob($(".sticky-active"), "active", "will-sticky");
      function stickyMenuMob($targetMenu, $toggleClass, $parentClass) {
        var st = $(window).scrollTop();
        var height = $targetMenu.css("height");
        $targetMenu.parent().css("min-height", height);
      
        if ($(window).scrollTop() > 100) {
          $(headerTop).addClass("header-top-mob");
          $targetMenu.parent().addClass($parentClass);
          $targetMenu.addClass($toggleClass);
        } else {
          $(headerTop).removeClass("header-top-mob");
  ...
  $('.services-carousel').slick({
    ...
    slidesToScroll: 1,
    dots: true,
    prevArrow: '<button class="slick-prev"><span aria-label="Previous">‹</span></button>',
    nextArrow: '<button class="slick-next"><span aria-label="Next">›</span></button>',
    responsive: [
      {
        breakpoint: 1024,
        settings: {
